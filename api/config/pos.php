...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Default rate applied to a table's bill when a service charge is
    | requested without an explicit override rate.
    |
    */
    'service_charge_rate' => (float) env('POS_SERVICE_CHARGE_RATE', 0.10),

    /*
    |--------------------------------------------------------------------------
    | Drawer variance threshold
    |--------------------------------------------------------------------------
    |
    | A drawer count whose variance (actual minus expected cash) is at or
    | beyond this magnitude puts the shift under manager review before the
    | till can be closed.
    |
    */
    'till_variance_manager_threshold' => (float) env('POS_TILL_VARIANCE_MANAGER_THRESHOLD', 5),
];
